...
// With Adaptive Sleep Mode based on Plant Health
// ========================================
...
async function fetchUserAdvice() {
    try {
        ...
        // Update device health scores from advice response
        if (data.health_scores_updated && data.health_scores_updated.length > 0) {
            console.log('Health scores updated:', data.health_scores_updated);
            // Refresh device data to get updated health scores
            userDevices = await fetchUserDevices();
            updateDeviceCards();
        }
...
// ========================================
// Adaptive Sleep Helper Functions
// ========================================

function calculateAdaptiveSleepDuration(healthScore) {
    /**
     * Calculate sleep duration in seconds based on plant health score.
     * Mirrors the backend calculation for display purposes.
     * 
     * @param {number} healthScore - Plant health score (0-10)
    ...
    if (score <= 2) {
        ...
        return Math.round(45 + (score - 2) * 7.5);
    } else if (score <= 6) {
        return Math.round(60 + (score - 4) * 60);
    } else if (score <= 8) {
        return Math.round(180 + (score - 6) * 210);
    } else {
        return Math.round(600 + (score - 8) * 600);
    }
}

function formatSleepDuration(seconds) {
    /**
     * Format sleep duration in human-readable format.
     * @param {number} seconds - Duration in seconds
     * @returns {string} Formatted duration (e.g., "30s", "2m", "15m")
     */
    if (seconds < 60) {
        return `${seconds}s`;
...
function getHealthScoreLabel(score) {
    /**
     * Get a descriptive label for a health score.
...
function getHealthScoreClass(score) {
    /**
     * Get CSS class for health score styling.
     * @param {number} score - Health score (0-10)
     * @returns {string} CSS class name
     */
    if (score === null || score === undefined) return 'health-unknown';
    if (score <= 2) return 'health-critical';
    if (score <= 4) return 'health-poor';
    if (score <= 6) return 'health-fair';
...
async function loadUserAdvice() {
    ...
    try {
        ...
        // Refresh device cards to show updated health scores
        updateDeviceCards();
...
function createDeviceCard(device, latestReading, readings) {
    ...
    // Determine current sampling rate and adaptive mode
    const currentInterval = device.target_interval || 30;
    const adaptiveEnabled = device.adaptive_sleep_enabled || false;
    const healthScore = device.plant_health_score;
    const hasHealthScore = healthScore !== null && healthScore !== undefined;
    
    // Calculate adaptive sleep duration if health score exists
    const adaptiveSleep = hasHealthScore ? calculateAdaptiveSleepDuration(healthScore) : null;
    ...
    card.innerHTML = `
        ...
        <div class="health-score-container ${hasHealthScore ? '' : 'hidden'}" id="health-container-${device.device_id}">
            <div class="health-score-badge ${getHealthScoreClass(healthScore)}">
                <span class="health-score-value">${hasHealthScore ? healthScore.toFixed(1) : '--'}</span>
                <span class="health-score-label">${getHealthScoreLabel(healthScore)}</span>
            </div>
            <div class="adaptive-sleep-info ${adaptiveEnabled ? '' : 'hidden'}" id="adaptive-info-${device.device_id}">
                <span class="adaptive-sleep-icon">⏱️</span>
                <span class="adaptive-sleep-text">
                    ${adaptiveSleep ? `Sampling: ${formatSleepDuration(adaptiveSleep)}` : 'Calculating...'}
                </span>
            </div>
        </div>
        ...
        <div class="chart-controls">
            ...
            <div class="control-group">
                <label for="sleep-mode-${device.device_id}">Sleep Mode:</label>
                <select id="sleep-mode-${device.device_id}" class="sleep-mode-select" data-device-id="${device.device_id}">
                    <option value="manual" ${!adaptiveEnabled ? 'selected' : ''}>Manual</option>
                    <option value="adaptive" ${adaptiveEnabled ? 'selected' : ''}>Auto (Health)</option>
                </select>
            </div>
            <div class="control-group sampling-rate-group ${adaptiveEnabled ? 'disabled-group' : ''}" id="sampling-group-${device.device_id}">
                <label for="sampling-rate-${device.device_id}">Sampling:</label>
                <select id="sampling-rate-${device.device_id}" class="sampling-rate-select" data-device-id="${device.device_id}" ${adaptiveEnabled ? 'disabled' : ''}>
...
function setupEventListeners() {
    ...
    // Time range, sleep mode, and sampling rate selectors (delegated event listener for dynamic elements)
    document.addEventListener('change', async (e) => {
        if (e.target.classList.contains('time-range-select')) {
        ...
        } else if (e.target.classList.contains('sleep-mode-select')) {
            // Handle sleep mode toggle (Manual vs Adaptive)
            const deviceId = e.target.dataset.deviceId;
            const mode = e.target.value;
            const adaptiveEnabled = (mode === 'adaptive');
            ...
            try {
                ...
                const response = await fetch(`${CONFIG.apiBaseUrl}/devices/${deviceId}/config`, {
                    method: 'POST',
                    headers: headers,
                    body: JSON.stringify({ adaptive_sleep_enabled: adaptiveEnabled })
                });
                
                if (!response.ok) {
                    throw new Error('Failed to update sleep mode');
                }
                
                const result = await response.json();
                
                // Update UI based on mode
                const samplingGroup = document.getElementById(`sampling-group-${deviceId}`);
                const samplingSelect = document.getElementById(`sampling-rate-${deviceId}`);
                const adaptiveInfo = document.getElementById(`adaptive-info-${deviceId}`);
                const healthContainer = document.getElementById(`health-container-${deviceId}`);
                
                if (adaptiveEnabled) {
                    // Disable manual sampling rate selection
                    if (samplingGroup) samplingGroup.classList.add('disabled-group');
                    if (samplingSelect) samplingSelect.disabled = true;
                    if (adaptiveInfo) adaptiveInfo.classList.remove('hidden');
                    
                    // Show calculated sleep if available
                    if (result.calculated_adaptive_sleep && adaptiveInfo) {
                        const sleepText = adaptiveInfo.querySelector('.adaptive-sleep-text');
                        if (sleepText) {
                            sleepText.textContent = `Sampling: ${formatSleepDuration(result.calculated_adaptive_sleep)}`;
                        }
                    }
                } else {
                    // Enable manual sampling rate selection
                    if (samplingGroup) samplingGroup.classList.remove('disabled-group');
                    if (samplingSelect) samplingSelect.disabled = false;
                    if (adaptiveInfo) adaptiveInfo.classList.add('hidden');
                }
                
                // Show success
                e.target.options[e.target.selectedIndex].text = 'Saved!';
                setTimeout(() => {
                    e.target.options[e.target.selectedIndex].text = adaptiveEnabled ? 'Auto (Health)' : 'Manual';
                    e.target.disabled = false;
                }, 1000);
                
                console.log(`Sleep mode updated for ${deviceId}: ${mode}`);
                
            } catch (error) {
                console.error('Error updating sleep mode:', error);
                alert('Failed to update sleep mode. Please try again.');
                // Revert selection
                e.target.value = adaptiveEnabled ? 'manual' : 'adaptive';
                e.target.disabled = false;
            }
...
window.GrowSense = {
    ...
    calculateAdaptiveSleepDuration,
    formatSleepDuration
};
