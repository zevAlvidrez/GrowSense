...
NEW: Health scores from advice are used to calculate adaptive sleep intervals for ESP32 devices.
"""
...
def calculate_adaptive_sleep_duration(health_score):
    ...
    This function mirrors the one in routes.py for consistency.
...
def priority_to_health_score(priority):
    """
    Convert Gemini advice priority to a health score.
    
    Args:
        priority: String priority level ('urgent', 'high', 'medium', 'low')
    
    Returns:
        int: Health score (0-10)
    """
    priority_mapping = {
        'urgent': 2,
        'high': 4,
    ...
    return priority_mapping.get(priority.lower() if priority else 'low', 7)


def save_health_score_to_device(device_id, health_score, user_id):
    """
    Save the plant health score to the device document in Firestore.
    This score will be used by routes.py to calculate adaptive sleep intervals.
    ...
    if not device_id or not user_id:
        print(f"Warning: Cannot save health score - missing device_id or user_id")
        return False
    
    try:
        db = get_firestore()
        
        # Update the user's device document with health score
        device_ref = db.collection('users').document(user_id).collection('devices').document(device_id)
        device_ref.update({
            'plant_health_score': health_score,
            'health_score_updated_at': firestore.SERVER_TIMESTAMP
        })
        
        # Also update global devices collection for cross-reference
        ...
        print(f"  -> Saved health score {health_score} to device {device_id} (adaptive sleep: {calculated_sleep}s)")
        return True
        
    except Exception as e:
        print(f"  -> ERROR: Could not save health score to device {device_id}: {e}")
        return False
                ...
                if advice.get('device_advice'):
                    ...
                    for da in advice['device_advice']:
                        ...
                        if device_id and device_id in valid_device_ids:
                            ...
                            # Ensure priority exists for health score calculation
                            if 'priority' not in da:
                                da['priority'] = 'low'
...
def construct_prompt(formatted_data, analysis_history=None):
    ...
    IMPORTANT: The prompt requests a priority field which is used to calculate
    health scores for adaptive sleep intervals.
...
IMPORTANT: The "priority" field is CRITICAL for determining how often the device monitors the plant:
- "urgent" = Plant needs immediate attention (device will check every 30 seconds)
- "high" = Plant has issues that need addressing soon (device checks every ~1 minute)
- "medium" = Plant could use some care (device checks every ~2-3 minutes)
- "low" = Plant is healthy (device checks every ~10-30 minutes to save battery)
...
IMPORTANT: 
1. Use the EXACT device_id and device_name from the device list above. Each device_advice entry MUST match a device from the list.
2. Replace [DEVICE_NAME] with actual device names in recommendations.
3. Set priority accurately - this determines monitoring frequency!
...
IMPORTANT: The "plant_health_score" is CRITICAL for determining how often the device monitors the plant:
- 0-2 = Critical health (device will check every 30 seconds)
- 3-4 = Poor health (device checks every ~1 minute)
- 5-6 = Fair health (device checks every ~2-3 minutes)
- 7-8 = Good health (device checks every ~5-10 minutes)
- 9-10 = Excellent health (device checks every ~15-30 minutes to save battery)
...
IMPORTANT: 
1. Use the EXACT device names from the device list above when referencing specific devices.
2. Replace [DEVICE_NAME] with actual device names.
3. Set plant_health_score accurately - this determines monitoring frequency!
...
def convert_api_to_analysis_format(api_response):
    ...
    """
    # Calculate health score from device priorities
    priorities = [da.get('priority', 'low') for da in api_response.get('device_advice', [])]
    if priorities:
        # Average the health scores from priorities
        scores = [priority_to_health_score(p) for p in priorities]
        avg_score = sum(scores) / len(scores)
    else:
        avg_score = 7  # Default middle score
    ...
    analysis_response = {
        ...
        "plant_health_score": round(avg_score),
...
def run_analysis(user_id, time_range_hours=24, limit_per_device=30):
    ...
    Also saves health scores to devices for adaptive sleep calculation.
    ...
    try:
        ...
        if analysis_result:
            # Print the results
            health_score = analysis_result.get('plant_health_score', 'N/A')
            ...
            # Calculate and display adaptive sleep info
            if isinstance(health_score, (int, float)):
                adaptive_sleep = calculate_adaptive_sleep_duration(health_score)
                print(f"\n  Adaptive Sleep Calculation:")
                print(f"    - Health Score: {health_score}")
                print(f"    - Calculated Sleep Duration: {adaptive_sleep} seconds")
            ...
            # Save health score to all devices for this user
            if isinstance(health_score, (int, float)):
                print("Updating device health scores for adaptive sleep...")
                for device in formatted_data.get('devices', []):
                    device_id = device.get('device_id')
                    if device_id:
                        save_health_score_to_device(device_id, health_score, user_id)
